...
// Ejercicio 1 Guardar en un array todos los username
// Ejercicio 2 Obtener el firstname last name de todos los usuarios cuyo email no sea.com crear una estructura de tipo map
// que guarde el first name y lastname con su email

//Ejercicio1
const sacarNombres= userJSON => userJSON.map(persona => persona.login.username);
console.log("traza");
console.log(sacarNombres(users));

//Ejercicio2
b=new Map();
const sacarNombreEmail=userJSON=> userJSON.filter(persona => !(persona.email.includes(".com"))).map(persona1=>(`${persona1.firstname} ${persona1.lastname} ${persona1.email}`)).map(persona2 => persona2.split(" ")).forEach(persona3  =>{b.set(`${persona3[0]+persona3[1]} `,persona3[2])});
sacarNombreEmail(users);
console.log(b);


//Ejercicio2 reduce

const sacarNombreEmail2=userJSON=> userJSON.filter(persona => !persona.email.includes(".com")).reduce((acumulador,persona1)=>acumulador.set(`${persona1.firstname}${persona1.lastname}`,persona1.email),new Map());
console.log(sacarNombreEmail2(users));

//Ejercicio 2 solo con un reduce

const sacarNombreEmail3=userJSON=>user.JSON.reduce((acumulador,persona)=>persona.endsWith(".es")?acumulador.set(persona.name,persona.email):null,new Map());